...
- `main.doc`: copy of the test / demo program used to validate the behavior of `_printf`. It is useful for checking edge cases and comparing the custom implementation with the standard `printf`. It should not be pushed in the root directory for the final submission if it contains a `main` function, because the grading system provides its own test file.
